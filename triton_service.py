...
def run_inference_retinaface(client: Any, image_bytes: bytes) -> dict:
    """
    Run RetinaFace ONNX inference via Triton.
    Returns raw boxes / scores / landmarks (no NMS).
    """
    from io import BytesIO
    from PIL import Image
    import numpy as np
    ...
    with Image.open(BytesIO(image_bytes)) as img:
        img = img.convert("RGB").resize(RETINAFACE_IMAGE_SIZE)
...
##TODO: maybe move the get biggest face out of this service

    boxes = response.as_numpy("bbox")[0]
    landmarks = response.as_numpy("landmark")[0]
    scores = response.as_numpy("confidence")[0]
